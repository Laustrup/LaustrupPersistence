...
public abstract class Repository {
    ...
    public Connection handleConnection() {
        try {
            if ((DbGate.connectionIsNull() || DbGate.isClosed().get_truth()) && DbGate.open())
                return DbGate.get_connection();
            else if (DbGate.isOpen().get_truth())
                return DbGate.get_connection();
        } catch (Exception e) {
            Printer.get_instance().print("Couldn't handle connection from Repository...",e);
        }
        return null;
    }
     ...
     */
    public Plato closeConnection() { return DbGate.close(); }
     ...
     */
    private Connection connection() { return DbGate.get_connection(); }
